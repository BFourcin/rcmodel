...
# Changing this would change the observation space, so it is rejected outright.
FROZEN_KEYS = ("step_length",)
...
class LSIEnv(gym.Env):
    ...
    Model-level plots are not drawn here: evaluate() can record one evaluation window's
    full trajectory, and rcmodel.tools.plotting draws it.
    """
    ...
    def __init__(self, config: dict):
        ...
        self._check_room_columns(self.dataloader.dataset)
    ...
    def _check_room_columns(self, dataset):
        """Raise unless the data's temperature columns are the model's rooms, in order.

        Rooms are matched to data columns by position (step() compares the first n_rooms
        columns), so a room list in a different order from the CSV would silently score
        each room against another room's measurements. The CSV headers are the only record
        of which column is which, so check them by name.
        """
        headings = getattr(dataset, "headings", None)
        if headings is None:
            return
        room_names = [room.name for room in self.RC.building.rooms]
        # Dataset rows are (date-time, time, <room columns>) - see BuildingTemperatureDataset.
        data_rooms = list(headings[2 : 2 + len(room_names)])
        if data_rooms != room_names:
            raise ValueError(
                f"Model rooms {room_names} do not match the data's first temperature columns "
                f"{data_rooms}. Rooms are matched to columns by position, so order room_names "
                f"to match the CSV (or reorder the CSV)."
            )

    def __getstate__(self):
        # DataLoader iterators don't pickle, and Ray pickles environments when it moves
        # them between processes. It is derived state that rebuilds itself on demand.
    ...
    def update_from_config(self, new_config=None):
        ...
        if dataloader is not None and dataloader is not self.dataloader:
            self._check_room_columns(dataloader.dataset)
